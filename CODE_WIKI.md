...
> **定位**：个人 Prompt 与 Skill 管理工具（保存 → 整理 → 搜索 → 复制使用）
...
RoBox 是一个面向个人的 Prompt 和 Skill 管理工具。产品边界严格限定在两类内容：
...
| `type` | text | NOT NULL, CHECK in (`prompt`, `skill`) | 内容类型 |
...
| `source_url` | text | nullable | 来源 URL（GitHub 导入时填写） |
...
| `type` | text | CHECK in (`prompt`, `skill`) | 适用内容类型 |
| `name` | text | NOT NULL, UNIQUE(user_id, type, name) | 分类名称 |
| `sort_order` | int | >= 0 | 排序权重 |
| `created_at` | timestamptz | auto | 创建时间 |

**关键设计**：每个用户的 Prompt 和 Skill 分类独立管理，新用户首次访问时自动 seed 8 个默认分类。
...
| Schema 名称 | 类型 | 关键约束 |
|------------|------|---------|
| `itemTypeSchema` | `z.enum(["prompt", "skill"])` | 内容类型枚举 |
