...
RoBox 是一个个人 Prompt / Skill / Tool 管理工具，用来保存、整理、搜索和快速复制你常用的 Prompt 模板、Skill 与工具链接。

核心定位很清晰：只做 `prompt`、`skill`、`tool` 三类内容，围绕「保存 → 整理 → 搜索 → 复制使用」这一闭环，不扩展为通用知识库、爬虫平台或 Agent 平台。
...
| **自定义分类** | 设置页按 Prompt/Skill/Tool 分别管理分类，支持增删和排序 |
| **智能整理** | 基于 DeepSeek 自动提取标题、摘要、分类、标签和 Prompt 变量 |
| **变量填充** | Prompt 详情页支持填写变量，预览并复制最终文本 |
| **GitHub 导入** | 粘贴 GitHub 仓库链接，自动抓取 README 并整理为 Skill 或 Tool |
| **网站导入** | 粘贴公共 HTTPS 工具网站，抓取页面文本用于 Tool 智能整理 |
...
- **认证**：Supabase Auth (GitHub OAuth + Magic Link) + 邮箱白名单
...
│   └── api/                # Route Handlers (analyze / import/github / import/web)
├── components/             # 页面组件与共享 UI
├── hooks/                  # 客户端共享 hooks (toast 等)
├── features/items/         # Prompt / Skill / Tool 类型与查询参数解析
...
│   └── import/             # GitHub/网页 URL 校验、内容抓取、导入编排
```

数据模型围绕四张表：

- `items` — Prompt / Skill / Tool 统一实体
...
云 Supabase 的 Auth URL 配置通过 Dashboard 或 Management API 管理（**禁止 `supabase config push`**，会覆盖云配置）：

- Site URL: `https://robox-beta.vercel.app`
- Redirect URLs: `https://robox-beta.vercel.app/auth/confirm`
- GitHub OAuth: Dashboard → Authentication → Providers → GitHub，填入 Client ID / Secret
...
| `docs/architecture.md` | 系统边界、数据流、DeepSeek 整理链路、GitHub/网页导入链路、安全层 |
