...
- `2026-05-05`：全面性能优化——workspace 骨架屏消除空白等待、详情页 Suspense 流式加载、数据库 3 个 RPC 原子操作（`toggle_favorite`/`increment_usage_count`/`get_latest_copied_at`）、Dashboard 并行查询+limit、2 个新索引（`is_favorite` 复合+`title` pg_trgm GIN）、React.memo 减少重渲染、批量分析并发、DeleteItemButton bug 修复、LibraryFilters 客户端导航、缓存头优化
...
- `src/lib`：环境变量、导航、Supabase 客户端工厂、校验 schema、速率限制、格式化工具、通用工具
